...
const UrlImportTab: React.FC<UrlImportTabProps> = ({
  ...
  const supportedPlatforms = [
    ...
    { name: 'Direct URL', icon: '🔗', example: 'https://example.com/video.mp4' },
  ]
    ...
    <div className="space-y-6">
      <div className="space-y-4">
        <div className="text-center space-y-2">
          ...
          <h3 className="text-lg font-medium text-text-primary">Import from URL</h3>
        ...
        <div className="space-y-2">
          <label htmlFor="url-input" className="block text-sm font-medium text-text-primary">
            Video URL
          </label>
      ...
      <div className="border-t border-border pt-6">
        <h4 className="text-sm font-medium text-text-primary mb-4">Supported Platforms</h4>
        <div className="grid gap-3">
          {supportedPlatforms.map((platform, index) => (
            <div key={index} className="flex items-center space-x-3 p-3 bg-surface-secondary rounded-lg border border-border">
              <span className="text-2xl">{platform.icon}</span>
              <div>
                <p className="text-sm font-medium text-text-primary">{platform.name}</p>
                <p className="text-xs text-text-secondary">{platform.example}</p>
              </div>
            </div>
          ))}
        </div>
