...
const Tab: React.FC<TabProps> = ({
  ...
  useEffect(() => {
    if (orientation === 'horizontal' && !isQuiet && tabsRef.current) {
      const updateSliderPosition = () => {
        const selectedButton = tabsRef.current?.querySelector(`[aria-selected="true"]`) as HTMLElement;
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    ...
    const currentIndex = tabElements.findIndex(child => 
      isValidElement(child) && (child.props as TabItemProps).id === actualSelectedId
    );
  ...
  const processedChildren = Children.map(children, (child) => {
    ...
    return cloneElement(child as React.ReactElement<TabItemProps>, {
      ...
      className: isFluid && orientation === 'horizontal' 
